...
$existingGoal = find_saving_goal($conn, $userId, $goalId);
...
$goal = [
    'name' => trim($_POST['name'] ?? $existingGoal['name']),
    'target_amount' => trim($_POST['target_amount'] ?? $existingGoal['target_amount']),
    'current_amount' => trim($_POST['current_amount'] ?? $existingGoal['current_amount']),
    'target_date' => $_POST['target_date'] ?? $existingGoal['target_date'],
    'status' => $_POST['status'] ?? $existingGoal['status'],
];
$errors = [];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        db_execute(
            $conn,
            'UPDATE saving_goals SET name = ?, target_amount = ?, current_amount = ?, target_date = ?, status = ? WHERE id = ? AND userid = ?',
            'sddssii',
            [$goal['name'], $targetAmount, $currentAmount, $goal['target_date'], $status, $goalId, $userId]
        );

        sync_saving_goal_status($conn, $userId, $goalId);
        flash('success', 'Saving goal updated.');
        redirect(app_url('savings/show.php?id=' . $goalId));
    }
}

$pageTitle = 'Edit Saving Goal - Finote';
$activePage = 'savings';
$navUser = $user;
...
<main class="app-main">
    <div class="container">
        <div class="saving-card app-card p-4">
            <h1 class="page-title h3 mb-1">Edit Saving Goal</h1>
            <p class="text-muted mb-4">Update the goal details and progress.</p>
